...
if __name__ == '__main__':
    ...
    filename1 = './images/youngEW.jpg'
    filename2 = './images/oldEW.jpg'
    alphaList = []
    for n in range(51):
    ...
    cv2.imshow('original image 1', img1_orig)
    cv2.imshow('original image 2', img2_orig)
    ...
    cv2.imshow('image 1 with voronoi diagram', img1_voronoi)

    img2_voronoi = np.zeros(img2.shape, dtype=img2.dtype)

    draw_voronoi(img2_voronoi, subdiv2)
    cv2.imshow('image 2 voronoi with voronoi diagram', img2_voronoi)
    videoOut = cv2.VideoWriter("myVideo.mp4", cv2.VideoWriter_fourcc(*'mp4v'), 30, (int(img1_orig.shape[1]), int(img1_orig.shape[0])))
