...
int main(int argc, char** argv) {
    ...
    static float_t ks[3] = { 1.0f, 1.0f ,1.0f };

    //create buffer to initial default uniform value which will be used in shader 
    std::vector<std::string> uniformname = { "u_lightDir","KS"};
    std::vector<void*> uniformdata = { &lightDir[0],&ks[0]};
  ...
  while (glfwWindowShouldClose(window) == 0) {
...

    {
        ...
        ImGui::SliderFloat3("KS.", &ks[0], 0.0f, 1.0f);
        ...
        if (selectItem==0) {
            ...
            glUniform3fv(voxShader.getShaderUniformLocation("KS"), 1, ks);
        ...
        if (selectItem == 1 || selectItem == 2) {
            ...
            glUniform3fv(objShader.getShaderUniformLocation("KS"), 1, ks);

            toonShader.bindShaderProgram();
            glUniform3fv(toonShader.getShaderUniformLocation("KS"), 1, ks);
